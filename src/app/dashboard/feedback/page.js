...
import { fetchQuestionsByUser } from "@/app/services/firestore";
    ...
    <div className="m-3 bg-gray-200 rounded-lg shadow-sm  w-[96%] flex gap-3">
      ...
      <div className="h-full w-full">
        {messages.map((message)=>(
          ...
          <p className="font-semibold text-[10px] text-black">{message.category}</p>
          <h3 className="text-[12px] text-gray-500">{message.date.toDate().toDateString()}</h3>
          </div>
            <p className="text-gray-400 text-10 whitespace-nowrap overflow-hidden text-ellipsis w-[300px]">{message.message}</p>
         </div>
        ))}
      </div>
        )}
