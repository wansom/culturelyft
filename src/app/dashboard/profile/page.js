 ...
 const handleProgressChange=(percent,stage)=>{
  ...
  const uploadFile = async (file) => {
    ...
    try {
      ...
      console.log('find attached the url',url);
  ...
  <img src={user.photo} className="rounded-full h-40 w-40"/>
      )}
      {
        !user.photo&&( <label
          htmlFor="cr12"
          className="rounded-full h-40 w-40 bg-gray-200 mt-10 hover:bg-gray-100  flex flex-col items-center justify-center"
        >
       {!fileloading&&(
<>
  Upload Picture
          <input
            type="file"
            id="cr12"
            placeholder="Browse to upload your logo"
...
<div class="animate-spin rounded-full h-5 w-5 border-b-2 border-black"></div>
                ...
                <p className="text-center mx-6 text-[12px] ">Photo should resemble passport or ID for Account verifcation</p>
